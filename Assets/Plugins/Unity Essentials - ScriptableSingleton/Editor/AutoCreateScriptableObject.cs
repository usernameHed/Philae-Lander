...
namespace UnityEssentials.ScriptableObjectSingleton
{
    ...
    public static class AutoCreateScriptableObject
    {
        ...
        {
            CreateFmodPathAssetIfAny("ExampleScriptable", "Assets/Unity Essentials - ScriptableSingleton/Resources/");
        }

        private static void CreateFmodPathAssetIfAny(string assetName, string folderParent)
        {
            if (ExampleScriptable.Instance == null)
            {
                ExampleScriptable example = ScriptableObject.CreateInstance<ExampleScriptable>();
                CreateEntirePathIfNotExist(folderParent);
                Debug.Log("Generate asset: " + example, example);
                AssetDatabase.CreateAsset(example, folderParent + assetName + ".asset");
            }
        }
