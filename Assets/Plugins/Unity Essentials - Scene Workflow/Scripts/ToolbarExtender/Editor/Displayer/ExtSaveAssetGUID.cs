...
namespace UnityEssentials.SceneWorkflow.toolbarExtent
{
    ...
    public class ExtSaveAssetGUID
    {
        ...
        public static void SaveAllAssetReference(List<UnityEngine.Object> assets, string key, List<string> additionnalJsonDatas)
        {
            ...
            {
                guidAssets = new ListContainer(new List<string>(assets.Count));
                guidAssetsAdditionnalData = new ListContainer(new List<string>(assets.Count));
            }

            for (int i = 0; i < assets.Count; i++)
            {
                //if this object is already saved, just update the additionnalData
                string currentGuidAsset = AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(assets[i]));
                if (ContainIndex(guidAssets.List, currentGuidAsset, out int index))
                {
                    guidAssetsAdditionnalData.List[index] = additionnalJsonDatas[i];
                }
                else
                {
                    guidAssets.List.Add(currentGuidAsset);
                    guidAssetsAdditionnalData.List.Add(additionnalJsonDatas[i]);
                }
            }
        ...
        {
            if (!EditorPrefs.HasKey(key) || asset == null)
            {
                return ("");
            }
            ListContainer guidAssets = JsonUtility.FromJson<ListContainer>(EditorPrefs.GetString(key));
