...
namespace UnityEssentials.Spline.editor
{
    ...
    public class TriangleEditor : BaseEditor<Triangle>
    {
        [MenuItem("GameObject/Unity Essentials/Splines/Spline Type/Triangle", false, 10)]
        public static Triangle CreateTriangle()
        {
            GameObject splineGameObject = new GameObject("Triangle");
            Triangle line = splineGameObject.AddComponent<Triangle>();

            Undo.RegisterCreatedObjectUndo(splineGameObject, "Create Triangle");
            ...
            Editor splineEditorGeneric = CreateEditor(line, typeof(TriangleEditor));
            TriangleEditor splineEditor = (TriangleEditor)splineEditorGeneric;
            splineEditor.ConstructTriangle();
            return (line);
        }

        public static Triangle CreateLineOnGameObject(GameObject splineGameObject)
        {
            Triangle line = splineGameObject.AddComponent<Triangle>();
            
            Undo.RegisterCompleteObjectUndo(splineGameObject, "Create Triangle");
            Selection.activeGameObject = splineGameObject;
            SceneView.lastActiveSceneView.MoveToView(splineGameObject.transform);
            GameObjectUtility.EnsureUniqueNameForSibling(splineGameObject);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_waypointList == null || _waypointList.count != 3)
            {
                SetupWaypointList();
            }
        ...
        void SetupWaypointList()
        {
            ...
            if (_waypointList.count == 0)
            {
                ...
                InsertWaypointAtIndexAndPosition(_waypointList.index, new Vector3(1.5f, 0, 0.8f));
                _waypointList.index = 2;

                _target.Resolution = 1;
                _target.Appearances.Width = 0.005f;
                _target.m_Looped = true;
            }
        ...
        private void InsertWaypointAtIndex(int indexA)
        {
