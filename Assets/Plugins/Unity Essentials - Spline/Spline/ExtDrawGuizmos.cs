...
namespace UnityEssentials.Spline
{
    ...
    public static class ExtDrawGuizmos
    {
        ...
        public static void DebugCylinderWithoutBase(Vector3 start, Vector3 end, Color color, float radius = 1, float duration = 0, bool depthTest = true)
        {
            Vector3 up = (end - start).normalized * radius;
            Vector3 forward = Vector3.Slerp(up, -up, 0.5f);
            Vector3 right = Vector3.Cross(up, forward).normalized * radius;
        ...
        public static void DebugHalfCapsuleWithoutBaseFromInsidePoint(Vector3 p1, Vector3 p2, Color color, float radius = 1, float duration = 0, bool depthTest = true, bool drawPoint = false)
        {
            Vector3 extremityP1 = p1 + (p1 - p2).normalized * radius;
            Vector3 extremityP2 = p2 - (p1 - p2).normalized * radius;
            DebugHalfCapsuleWithoutBase(extremityP1, extremityP2, color, radius, duration, depthTest, drawPoint);
        }
        public static void DebugHalfCapsuleWithoutBase(Vector3 start, Vector3 end, Color color, float radius = 1, float duration = 0, bool depthTest = true, bool drawPoint = true)
        {
            Vector3 up = (end - start).normalized * radius;
            Vector3 forward = Vector3.Slerp(up, -up, 0.5f);
            Vector3 right = Vector3.Cross(up, forward).normalized * radius;
        ...
        public static void DrawBezier(Vector3 A, Vector3 B, Vector3 defaultDir, Color color, float percent = 0.5f)
        {
            Vector3 tangent = defaultDir.normalized * (A - B).magnitude / 1 * percent;
            Handles.DrawBezier(A, B, A + tangent, B - tangent, color, EditorGUIUtility.whiteTexture, 1f);
        }
#endif
        #endregion
    }
