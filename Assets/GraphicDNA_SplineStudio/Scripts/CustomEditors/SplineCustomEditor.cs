...
public class SplineCustomEditor : Editor
{
 ...
 //private bool ReadyForClone = true;
    ...
    private bool OnInspectorGUI_Buttons(Spline bb)
    {
        ...
        {
            Undo.RecordObject(bb, "Tessellate x2");
        ...
        {
            Undo.RecordObject(bb, "Detach Points");
        ...
        {
            Undo.RecordObject(bb, "Reverse Points");
        ...
        if (GUILayout.Button(new GUIContent("    Genere Mesh   "), GUILayout.Height(buttonHeight)))
        {
            bb.GenereMesh();
    ...
    public override void OnInspectorGUI()
    {
        ...
        anyChange |= UITools.FloatField(serializedObject, "Ratio Size Points", bb.ratioSizePoints, (obj, value) => { (obj as Spline).ratioSizePoints = value; });
    ...
    private bool OnSceneGuiMove(Spline bb)
    {
        ...
        if(selectedSubEntities.Count == 1 && bb.CurveType == Spline.eSplineType.Bezier)
        {
            ...
            if (idx < bb.ControlPoints.Count - 1 || bb.IsClosed)
            {
                ...
                newPositionHandle = CustomHandles.PositionHandle(handlePoint, Quaternion.identity, size, false, bb.EditHandlesXZ, bb.EditHandlesY);
                if(newPositionHandle != handlePoint)
                {
                    Undo.RecordObject(bb, "Change Control Point Handle");
        ...
        if (newPositionWorld != centerWorld)
        {
            ...
            {
                anyChange = true;
                Undo.RecordObject(bb, "Move Control Points");
                int i = -1;
...
//ReadyForClone = false;
    ...
    private bool OnSceneGuiRotate(Spline bb)
    {
        ...
        if (newRotation != Quaternion.identity &&  eventType == EventType.MouseDrag && Event.current.button == 0)
        {
            ...
            {
                anyChange = true;
                Undo.RecordObject(bb, "Rotate Control Points");
                int i = -1;
                foreach (ControlPoint p in bb.ControlPoints)
                {
                    i++;
    ...
    private bool OnSceneGuiScale(Spline bb)
    {
        ...
        if (dif != 0)
        {
            ...
            anyChange = true;
            Undo.RecordObject(bb, "Scale Control Points");
            int i = -1;
            foreach (ControlPoint p in bb.ControlPoints)
            {
                i++;
        ...
        */
        ...
        if (subObjectMode || bb.AlwaysRenderControlPoints)
        {
            ...
            for (i = 0; i < bb.ControlPoints.Count; i++)
            {
                ...
                float size = Mathf.Min(0.5f, 0.2f * HandleUtility.GetHandleSize(p1)) * bb.ratioSizePoints;
                float handleSize = Mathf.Min(0.35f, 0.125f * HandleUtility.GetHandleSize(p1)) * bb.ratioSizePoints;
            ...
            if(pointPairsGray.Count > 0)
                GLRendering.DrawLines(pointPairsGray.ToArray(), Color.green);
    ...
    public void PresEscape()
    {
        Debug.Log("ici escape !!!");
        subObjectMode = false;
        this.Repaint();
    }
    ...
    private bool ProcessMouseEventsManualCreationModeRefine(int controlId)
    {
        ...
        if(bb.MouseCast(v, bb.transform, SceneView.lastActiveSceneView.camera, out hitPoint, out startIdx))
        {
            ...
            float size = 0.125f * HandleUtility.GetHandleSize(hitPoint) * (bb.ratioSizePoints / 3);
        ...
        if (Event.current.button == 0)
        {
            if (Event.current.GetTypeForControl(controlId) == EventType.MouseUp)
            {
                if (mouseDown)
                {
                    if (haveHit)
                    {
                        Undo.RecordObject(bb, "Add Billboard");
