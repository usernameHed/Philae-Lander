...
namespace UnityEditor
{
    ...
    /// </summary>
    public sealed class CustomHandles
    {
      ...
      Vector3.zero
        ...
        public static bool lighting
        {
            get
            {
                return CustomHandles.s_Lighting;
            }
            set
            {
                CustomHandles.s_Lighting = value;
            }
        }      
        ...
        public static Matrix4x4 matrix
        {
            get
            {
                return CustomHandles.s_Matrix;
            }
            set
            {
                CustomHandles.s_Matrix = value;
                CustomHandles.s_InverseMatrix = value.inverse;
            }
        }
        ...
        public static Matrix4x4 inverseMatrix
        {
            get
            {
                return CustomHandles.s_InverseMatrix;
            }
        }      
        ...
        {
            return CustomHandles.DoPositionHandle(position, rotation, pHandleSize, pIsObjSelected, pAllowXZ, pAllowY);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        public static Vector3 DoPositionHandle(Vector3 position, Quaternion rotation, float pHandleSize, bool pIsObjSelected, bool pAllowXZ, bool pAllowY)
        {
            ...
            switch (current.type)
            {
                case EventType.KeyDown:
                    if (current.keyCode == KeyCode.V && !CustomHandles.currentlyDragging)
                    {
                        CustomHandles.s_FreeMoveMode = true;
                        break;
                    }
                    break;
                case EventType.KeyUp:
                    position = CustomHandles.DoPositionHandle_Internal(position, rotation, pHandleSize, pIsObjSelected, pAllowXZ, pAllowY);
                    if (current.keyCode == KeyCode.V && !current.shift && !CustomHandles.currentlyDragging)
                        CustomHandles.s_FreeMoveMode = false;
                    return position;
                case EventType.Layout:
                    if (!CustomHandles.currentlyDragging)// && !Tools.vertexDragging)
                    {
                        CustomHandles.s_FreeMoveMode = current.shift;
                        break;
                    }
                    break;
            }
            return CustomHandles.DoPositionHandle_Internal(position, rotation, pHandleSize, pIsObjSelected, pAllowXZ, pAllowY);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        private static Vector3 DoPositionHandle_Internal(Vector3 position, Quaternion rotation, float pHandleSize, bool pIsObjSelected, bool pAllowXZ, bool pAllowY)
        {
            ...
            if (pAllowXZ && pAllowY)
            {
                Handles.color = !flag ? CustomHandles.XAxisColor : Color.Lerp(CustomHandles.XAxisColor, CustomHandles.staticColor, CustomHandles.staticBlend);
                GUI.SetNextControlName("xAxis");
                position = Handles.Slider(position, rotation * Vector3.right, handleSize, new Handles.CapFunction(Handles.ArrowHandleCap), SnapSettings.move.x);
                Handles.color = !flag ? CustomHandles.YAxisColor : Color.Lerp(CustomHandles.YAxisColor, CustomHandles.staticColor, CustomHandles.staticBlend);
                GUI.SetNextControlName("yAxis");
                position = Handles.Slider(position, rotation * Vector3.up, handleSize, new Handles.CapFunction(Handles.ArrowHandleCap), SnapSettings.move.y);
                Handles.color = !flag ? CustomHandles.ZAxisColor : Color.Lerp(CustomHandles.ZAxisColor, CustomHandles.staticColor, CustomHandles.staticBlend);
                GUI.SetNextControlName("zAxis");
                position = Handles.Slider(position, rotation * Vector3.forward, handleSize, new Handles.CapFunction(Handles.ArrowHandleCap), SnapSettings.move.z);
            }
            else if(pAllowXZ)
            {
                GUI.SetNextControlName("XZ Axis");
                position = Handles.Slider2D(position, rotation * Vector3.up, rotation * Vector3.forward, rotation * Vector3.right, handleSize, (Handles.CapFunction)null, SnapSettings.move.z);
            }
            else if(pAllowY)
            {
                Handles.color = !flag ? CustomHandles.YAxisColor : Color.Lerp(CustomHandles.YAxisColor, CustomHandles.staticColor, CustomHandles.staticBlend);
